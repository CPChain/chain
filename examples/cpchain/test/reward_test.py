...
def testBalance(node,instance,rewardBalance,lockBalance):
    getTotalBalance = instance.functions.getTotalBalance(node.cpc.accounts[0]).call()
    getLockBalance = instance.functions.getLockedBalance(node.cpc.accounts[0]).call()
    assert getTotalBalance > rewardBalance,\
        f'wrong getTotalBalance, getTotalBalance is {getTotalBalance},rewardBalance {rewardBalance}'
    assert getLockBalance == lockBalance,\
        f'wrong LockBalance, totalBalance: {rewardBalance},getLockBalance: {getLockBalance}'
...
def main():
    ...
    # test balance
    assert reward.functions.totalInvestAmount().call()==220000 * Ether,\
        'totalInvestAmount error'

    # test deposit
    testBalance(rnode1, rnode1_instance, 200000 * Ether, 0)
    testBalance(rnode2, rnode2_instance, 200000 * Ether, 200000 * Ether)
    testBalance(enode1, enode1_instance, 20000 * Ether, 0)
    testBalance(enode2, enode2_instance, 20000 * Ether, 20000 * Ether)
