...
func TestTransactionStatusLes2(t *testing.T) {
	...
	config.Journal = ""
	txpool := core.NewTxPool(config, configs.TestChainConfig, chain)
	pm.txpool = txpool
	peer, _ := newTestPeer(t, "peer", 2, pm, true)
	defer peer.close()
	...
	tx0, _ := types.SignTx(types.NewTransaction(0, acc1Addr, big.NewInt(10000), configs.TxGas, nil, nil), signer, testBankKey)
	test(tx0, true, txStatus{Status: core.TxStatusUnknown, Error: core.ErrUnderpriced.Error()})

	tx1, _ := types.SignTx(types.NewTransaction(0, acc1Addr, big.NewInt(10000), configs.TxGas, big.NewInt(100000000000), nil), signer, testBankKey)
	...
	tx2, _ := types.SignTx(types.NewTransaction(1, acc1Addr, big.NewInt(10000), configs.TxGas, big.NewInt(100000000000), nil), signer, testBankKey)
	tx3, _ := types.SignTx(types.NewTransaction(2, acc1Addr, big.NewInt(10000), configs.TxGas, big.NewInt(100000000000), nil), signer, testBankKey)
	...
	gchain, _ := core.GenerateChain(configs.TestChainConfig, chain.GetBlockByNumber(0), ethash.NewFaker(), db, remoteDB, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(configs.TestChainConfig, chain.GetBlockByNumber(0), ethash.NewFaker(), db, remoteDB, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
