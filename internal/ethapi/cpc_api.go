...
func (c *ChainStateReader) RegisterServer(s *grpc.Server) {
	cpcpb.RegisterChainStateReaderServer(s, c)
}
...
func (c *ChainStateReader) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	cpcpb.RegisterChainStateReaderHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

// GasPrice returns a suggestion for a gas price.
func (c *ChainStateReader) GasPrice(ctx context.Context, req *empty.Empty) (*commonpb.GasPrice, error) {
	price, err := c.b.SuggestPrice(ctx)
	return &commonpb.GasPrice{GasPrice:price.Int64()}, err
}
...
func (c *ChainStateReader) Syncing(ctx context.Context, req *empty.Empty) (*commonpb.SyncingInfo, error) {
	progress := c.b.Downloader().Progress()

	// Return not syncing if the synchronisation already completed
	if progress.CurrentBlock >= progress.HighestBlock {
		return &commonpb.SyncingInfo{IsSyncing:false}, nil
	}
	// Otherwise gather the block sync stats
	return &commonpb.SyncingInfo{
...
func (c *ChainReader) GetBlockCount(ctx context.Context, req *empty.Empty) (*commonpb.BlockNumber, error) {
	header, _ := c.b.HeaderByNumber(context.Background(), rpc.LatestBlockNumber) // latest header should always be available
	return &commonpb.BlockNumber{BlockNumber: header.Number.Int64()}, nil
}
...
func (c *ChainReader) RegisterServer(s *grpc.Server) {
	cpcpb.RegisterChainReaderServer(s, c)
}

// RegisterGateway register api to restfull json
func (c *ChainReader) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	cpcpb.RegisterChainReaderHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

// GetBalance returns the amount of wei for the given address in the state of the
// given block number. The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta
// block numbers are also allowed.
func (s *ChainReader) GetBalance(ctx context.Context, req *cpcpb.ChainReaderRequest) (*commonpb.Balance, error) {
	...
	return &commonpb.Balance{Balance: balance.String()}, state.Error()
}

// newGRPCTransaction returns a transaction that will serialize to the RPC
// representation, with the given location metadata set (if available).
func newGRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *commonpb.Transaction {
...
func newGRPCPendingTransaction(tx *types.Transaction) *commonpb.Transaction {
...
func newGRPCTransactionFromBlockIndex(b *types.Block, index uint64) *commonpb.Transaction {
	txs := b.Transactions()
...
func newGRPCTransactionFromBlockHash(b *types.Block, hash common.Hash) *commonpb.Transaction {
...
// transaction hashes.
func GRPCMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (*commonpb.Block, error) {
	...
	if inclTx {
		formatTx := func(tx *types.Transaction) (*commonpb.Transaction, error) {
			return &commonpb.Transaction{TxHash: tx.Hash().String()}, nil
		}
		if fullTx {
			formatTx = func(tx *types.Transaction) (*commonpb.Transaction, error) {
		...
		txs := b.Transactions()
		block.Transactions = make([]*commonpb.Transaction, len(txs))
		var err error
		for i, tx := range txs {
...
func (s *ChainReader) grpcOutputBlock(b *types.Block, inclTx bool, fullTx bool) (*commonpb.Block, error) {
...
func (s *ChainReader) GetBlockByNumber(ctx context.Context, req *cpcpb.ChainReaderRequest) (*commonpb.Block, error) {
	...
	return &commonpb.Block{}, err
}

// GetBlockByHash returns the requested block. When fullTx is true all transactions in the block are returned in full
// detail, otherwise only the transaction hash is returned.
func (s *ChainReader) GetBlockByHash(ctx context.Context, req *cpcpb.ChainReaderRequest) (*commonpb.Block, error) {
...
func (t *TransactionReader) RegisterServer(s *grpc.Server) {
	cpcpb.RegisterTransactionPoolReaderServer(s, t)
}

// RegisterGateway register api to restfull json
func (t *TransactionReader) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	cpcpb.RegisterTransactionPoolReaderHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

// GetBlockTransactionCountByNumber returns the number of transactions in the block with the given block number.
func (t *TransactionReader) GetTransactionCountByBlockNumber(ctx context.Context, blockNr *commonpb.BlockNumber) (*cpcpb.TransactionCount, error) {
	if block, _ := t.b.BlockByNumber(ctx, rpc.BlockNumber(blockNr.BlockNumber)); block != nil {
		return &cpcpb.TransactionCount{TransactionCount: uint64(len(block.Transactions()))}, nil
	}
	return &cpcpb.TransactionCount{}, nil
}

// GetBlockTransactionCountByHash returns the number of transactions in the block with the given hash.
func (t *TransactionReader) GetTransactionCountByBlockHash(ctx context.Context, blockHash *commonpb.BlockHash) (*cpcpb.TransactionCount, error) {
...
func (t *TransactionReader) GetTransactionByBlockNumberAndIndex(ctx context.Context, req *cpcpb.TransactionPoolReaderRequest) (*commonpb.Transaction, error) {
	...
	return &commonpb.Transaction{}, nil
}

// GetTransactionByBlockHashAndIndex returns the transaction for the given block hash and index.
func (t *TransactionReader) GetTransactionByBlockHashAndIndex(ctx context.Context, req *cpcpb.TransactionPoolReaderRequest) (*commonpb.Transaction, error) {
...
func (t *TransactionReader) GetRawTransactionByBlockNumberAndIndex(ctx context.Context, req *cpcpb.TransactionPoolReaderRequest) (*cpcpb.RawTransaction, error) {
	if block, _ := t.b.BlockByNumber(ctx, rpc.BlockNumber(req.BlockNumber)); block != nil {
		return &cpcpb.RawTransaction{RawTransaction: newRPCRawTransactionFromBlockIndex(block, req.Index)}, nil
	}
	return &cpcpb.RawTransaction{}, nil
}

// GetRawTransactionByBlockHashAndIndex returns the bytes of the transaction for the given block hash and index.
func (t *TransactionReader) GetRawTransactionByBlockHashAndIndex(ctx context.Context, req *cpcpb.TransactionPoolReaderRequest) (*cpcpb.RawTransaction, error) {
	if block, _ := t.b.GetBlock(ctx, common.HexToHash(req.BlockHash)); block != nil {
		return &cpcpb.RawTransaction{RawTransaction: newRPCRawTransactionFromBlockIndex(block, req.Index)}, nil
	}
	return &cpcpb.RawTransaction{}, nil
...
func (t *TransactionReader) GetTransactionCount(ctx context.Context, req *cpcpb.TransactionPoolReaderRequest) (*cpcpb.TransactionCount, error) {
	...
	return &cpcpb.TransactionCount{TransactionCount: nonce}, nil
}

// GetTransactionByHash returns the transaction for the given hash
func (t *TransactionReader) GetTransactionByHash(ctx context.Context, txHash *cpcpb.TransactionHash) (*commonpb.Transaction, error) {
...
func (t *TransactionReader) GetTransactionReceipt(ctx context.Context, txHash *cpcpb.TransactionHash) (*commonpb.Receipt, error) {
	tx, blockHash, blockNumber, index := rawdb.ReadTransaction(t.b.ChainDb(), common.HexToHash(txHash.TransactionHash))
	if tx == nil {
		return &commonpb.Receipt{}, nil
	}
	...
	result := &commonpb.Receipt{
		...
		Logs:             make([]*commonpb.Log, len(receipt.Logs)),
...
func (c *AccountReader) RegisterServer(s *grpc.Server) {
	cpcpb.RegisterAccountReaderServer(s, c)
}

// RegisterGateway register api to restfull json
func (c *AccountReader) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	cpcpb.RegisterAccountReaderHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

// Accounts returns the collection of accounts this node manages
func (s *AccountReader) Accounts(ctx context.Context, req *empty.Empty) (*commonpb.Addresses, error){
	// addresses := make([]common.Address, 0) // return [] instead of nil if empty
	addresses := &commonpb.Addresses{Addresses:make([]string, 0, 0)}
	for _, wallet := range s.am.Wallets() {
		for _, account := range wallet.Accounts() {
			addresses.Addresses = append(addresses.Addresses, account.Address.String())
		}
	}
