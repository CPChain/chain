...
func newDummyChain() consensus.ChainReader {
	_, _ = core.GenerateChain(configs.TestChainConfig, genesis, dpor.New(configs.ChainConfigInfo().Dpor, testdb), testdb, nil, 8, nil)
	chain, _ := core.NewBlockChain(testdb, nil, configs.TestChainConfig, dpor.New(configs.ChainConfigInfo().Dpor, testdb), vm.Config{}, nil, nil)
