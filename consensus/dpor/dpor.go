...
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, common.Address, error) {
	...
	log.Info(hex.Dump(leaderSig))
	log.Info(hex.Dump(signerSig))
...
func (c *Dpor) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Dpor) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func acceptSigs(header *types.Header, sigcache *lru.ARCCache, signers []common.Address) (bool, error) {
	...
	if uint64(numSigs) > pctAccept*viewLength {
		accept = true
	}
	return accept, nil
}
...
func (c *Dpor) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	log.Info("extra length:" + strconv.Itoa(len(header.Extra)))
...
func (c *Dpor) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
...
// with.
func (c *Dpor) Authorize(signer common.Address, signFn SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Dpor) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	header := block.Header()
	...
	pleader, psigner, err := ecrecover(pb.Header(), c.signatures)
	...
	log.Info("leader of pb:" + pleader.Hex())
	...
	*/
	...
	log.Info("sig length:" + strconv.Itoa(len(sighash)))
	log.Info("sealed dump:" + strconv.Itoa(len(header.Extra)))
	log.Info(hex.Dump(header.Extra))
...
// current signer.
func (c *Dpor) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Dpor) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "dpor",
		Version:   "1.0",
		Service:   &API{chain: chain, dpor: c},
		Public:    false,
	}}
}
