...
func Test_logDebugWithNumberAndHash(t *testing.T) {
	type args struct {
		msg    string
		number uint64
		hash   common.Hash
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{
			name: "1",
			args: args{
				msg:    "parent is nil when verifying the header",
				number: 1,
				hash:   common.Hash{},
			},
		},

		{
			name: "2",
			args: args{
				msg:    "parent's number is not equal to header.number when verifying the header",
				number: 1,
				hash:   common.Hash{},
				ctx:    []interface{}{"parent.number", 0},
			},
		},

		{
			name: "3",
			args: args{
				msg: "parent's hash is not equal to header.parentHash when verifying the header",

				number: 1,
				hash:   common.Hash{},
				ctx:    []interface{}{"parent.hash", common.Hash{}, "header.parentHash", common.Hash{}},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			logDebugWithNumberAndHash(tt.args.msg, tt.args.number, tt.args.hash, tt.args.ctx...)
		})
	}
}
