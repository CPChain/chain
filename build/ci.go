...
var (
	...
	// Files that end up in the cpchain-alltools*.zip archive.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "cpchain-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "cpchain.exe" {
			gethTool = file
		} else {
	...
	// first section contains the cpchain binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.cpchain.nsi", filepath.Join(*workdir, "cpchain.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("cpchain-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "cpchain.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	if *local {
		...
		os.Rename("cpchain.aar", filepath.Join(GOBIN, "cpchain.aar"))
		return
	}
	...
	archive := "cpchain-" + archiveBasename("android", env) + ".aar"
	os.Rename("cpchain.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "cpchain-" + archiveBasename("ios", env)
