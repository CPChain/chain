...
func tmpKeyStoreIface(t *testing.T, encrypted bool) (dir string, ks keyStore) {
	d, err := ioutil.TempDir("", "cpchain-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
	if encrypted {
...
// Test and utils for the key store tests in the cpchain JSON tests;
...
func testDecryptV3(test KeyStoreTestV3, t *testing.T) {
	privBytes, _, rsaPrivBytes, err := decryptKeyV3(&test.Json, test.Password)
	if err != nil {
		t.Fatal(err)
	}
...
func testDecryptV1(test KeyStoreTestV1, t *testing.T) {
	privBytes, _, _, err := decryptKeyV1(&test.Json, test.Password)
	if err != nil {
		t.Fatal(err)
	}
	privHex := hex.EncodeToString(privBytes)
