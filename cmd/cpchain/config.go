...
func updateRpcConfig(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.IsSet(flags.GrpcGatewayFlagName) {
		addr := strings.Split(ctx.String(flags.GrpcGatewayFlagName), ":")
		if len(addr) != 2 {
			log.Fatalf("Wrong number of arguments for --%v flag\n", flags.GrpcGatewayFlagName)
		}
		cfg.Grpc.GatewayHost = addr[0]
		cfg.Grpc.GatewayPort, _ = strconv.Atoi(addr[1])
	}
	if ctx.IsSet(flags.GrpcIpcDisableFlagName) {
		cfg.Grpc.IpcPath = ""
	}
