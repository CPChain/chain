...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	var (
		...
		config        = &configs.ChainConfig{DAOForkBlock: big.NewInt(1), DAOForkSupport: localForked}
		gspec         = &core.Genesis{Config: config}
		genesis       = gspec.MustCommit(db)
		blockchain, _ = core.NewBlockChain(db, nil, config, pow, vm.Config{}, nil, nil)
	)
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, evmux, new(testTxPool), pow, blockchain, db, common.Address{})
	if err != nil {
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&configs.ChainConfig{}, genesis, ethash.NewFaker(), db, nil, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
				block.SetExtra(configs.DAOForkBlockExtra)
			}
		})
