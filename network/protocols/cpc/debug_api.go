...
func (d *DebugDumper) RegisterServer(s *grpc.Server) {
    debugpb.RegisterDebugDumperServer(s, d)
}
...
func (d *DebugDumper) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
    debugpb.RegisterDebugDumperHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

func grpcDumpAccount(dumpAccount state.DumpAccount) *commonpb.DumpAccount {
    storage := make(map[string]string)
    for k, v := range dumpAccount.Storage {
        storage[k] = v
    }
    return &commonpb.DumpAccount{
...
func grpcDump(dump state.Dump) *commonpb.Dump{
    grpcDump := &commonpb.Dump{
        Root:dump.Root,
    }
...
func (api *DebugDumper) DumpBlock(ctx context.Context, blockNumber *commonpb.BlockNumber) (*commonpb.Dump, error) {
    ...
    if block == nil {
        return &commonpb.Dump{}, fmt.Errorf("block #%d not found", blockNumber.BlockNumber)
    }
    stateDb, err := api.c.BlockChain().StateAt(block.StateRoot())
    if err != nil {
        return &commonpb.Dump{}, err
    }
    return grpcDump(stateDb.RawDump()), nil
}
...
func (d *DebugManager) RegisterServer(s *grpc.Server) {
    debugpb.RegisterDebugManagerServer(s, d)
}
...
func (d *DebugManager) Preimage(ctx context.Context, hash *commonpb.Hash) (*debugpb.PreimageValue, error) {
    if preimage := rawdb.ReadPreimage(d.c.ChainDb(), common.HexToHash(hash.Hash)); preimage != nil {
        return &debugpb.PreimageValue{Preimage: preimage}, nil
    }
    return &debugpb.PreimageValue{}, errors.New("unknown preimage")
}

// GetBadBlocks returns a list of the last 'bad blocks' that the client has seen on the network
// and returns them as a JSON list of block-hashes
func (d *DebugManager) GetBadBlocks(ctx context.Context, req *empty.Empty) (*commonpb.BadBlockArgs, error) {
    blocks := d.c.BlockChain().BadBlocks()
    results := make([]*commonpb.BadBlockArg, len(blocks))

    var err error
    for i, block := range blocks {
        results[i] = &commonpb.BadBlockArg{
    ...
    return &commonpb.BadBlockArgs{BadBlockArgs:results}, nil
}

// StorageRangeAt returns the storage at the given block height and transaction index.
func (d *DebugManager) StorageRangeAt(ctx context.Context, req *debugpb.DebugManagerRequest) (*debugpb.StorageRangeResult, error) {
    // _, _, statedb, err := d.computeTxEnv(common.HexToHash(req.BlockHash), int(req.TxIndex), 0)
    // if err != nil {
    //     return &debugpb.StorageRangeResult{}, err
    // }
    // st := statedb.StorageTrie(common.HexToAddress(req.ContractAddress))
    // if st == nil {
    //     return &debugpb.StorageRangeResult{}, fmt.Errorf("account %x doesn't exist", common.HexToAddress(req.ContractAddress))
    // }
    // return grpcStorageRangeAt(st, req.KeyStart, int(req.MaxResult))
    return &debugpb.StorageRangeResult{}, nil
}

func grpcStorageRangeAt(st state.Trie, start []byte, maxResult int) (*debugpb.StorageRangeResult, error) {
    it := trie.NewIterator(st.NodeIterator(start))
    result := &debugpb.StorageRangeResult{StorageMap: make(map[string]*debugpb.StorageEntry)}
    for i := 0; i < maxResult && it.Next(); i++ {
        _, content, _, err := rlp.Split(it.Value)
        if err != nil {
            return &debugpb.StorageRangeResult{}, err
        }
        e := &debugpb.StorageEntry{Value: common.BytesToHash(content).String()}
        if preimage := st.GetKey(it.Key); preimage != nil {
            preimage := common.BytesToHash(preimage)
            e.Key = preimage.String()
        }
        result.StorageMap[common.BytesToHash(it.Key).String()] = e
    }
    // Add the 'next key' so clients can continue downloading.
    if it.Next() {
        next := common.BytesToHash(it.Key)
        result.NextKey = next.String()
    }
    return result, nil
}
...
func (d *DebugManager) GetModifiedAccountsByNumber(ctx context.Context, req *debugpb.DebugManagerRequest) (*commonpb.Addresses, error) {
    var startBlock, endBlock *types.Block

    startBlock = d.c.blockchain.GetBlockByNumber(req.StartNumber)
    if startBlock == nil {
        return &commonpb.Addresses{}, fmt.Errorf("start block %x not found", req.StartNumber)
    }
    ...
    if req.EndNumber == nil {
        ...
        if startBlock == nil {
            return &commonpb.Addresses{}, fmt.Errorf("block %x has no parent", endBlock.Number())
        }
    } else {
        endBlock = d.c.blockchain.GetBlockByNumber(req.EndNumber.Value)
        if endBlock == nil {
            return &commonpb.Addresses{}, fmt.Errorf("end block %d not found", req.EndNumber.Value)
        }
    }
...
//
// With one parameter, returns the list of accounts modified in the specified block.
func (d *DebugManager) GetModifiedAccountsByHash(ctx context.Context, req *debugpb.DebugManagerRequest) (*commonpb.Addresses, error) {
    var startBlock, endBlock *types.Block
    startBlock = d.c.blockchain.GetBlockByHash(common.HexToHash(req.StartHash))
    if startBlock == nil {
        return &commonpb.Addresses{}, fmt.Errorf("start block %s not found", req.StartHash)
    }
    ...
    if req.EndHash == nil {
        ...
        if endBlock == nil {
            return &commonpb.Addresses{}, fmt.Errorf("end block %s not found", req.EndHash.Value)
        }
    }
    return d.getModifiedAccounts(startBlock, endBlock)
}

func (d *DebugManager) getModifiedAccounts(startBlock, endBlock *types.Block) (*commonpb.Addresses, error) {
    if startBlock.Number().Uint64() >= endBlock.Number().Uint64() {
        return &commonpb.Addresses{}, fmt.Errorf("start block height (%d) must be less than end block height (%d)", startBlock.Number().Uint64(), endBlock.Number().Uint64())
    }

    oldTrie, err := trie.NewSecure(startBlock.StateRoot(), trie.NewDatabase(d.c.chainDb), 0)
    if err != nil {
        return &commonpb.Addresses{}, err
    }
    ...
    var dirty = &commonpb.Addresses{Addresses:make([]string, 0, 0)}
    for iter.Next() {
        key := newTrie.GetKey(iter.Key)
        if key == nil {
            return &commonpb.Addresses{}, fmt.Errorf("no preimage found for hash %x", iter.Key)
        }
        dirty.Addresses = append(dirty.Addresses, common.BytesToAddress(key).String())
    }
    return dirty, nil
}
