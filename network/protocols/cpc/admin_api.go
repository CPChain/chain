...
func (c *ChainManager) RegisterServer(s *grpc.Server) {
    adminpb.RegisterAdminManagerServer(s, c)
}
...
func (c *ChainManager) RegisterGateway(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
    adminpb.RegisterAdminManagerHandlerFromEndpoint(ctx, mux, endpoint, opts)
}

// ExportChain exports the current blockchain into a local file.
func (api *ChainManager) ExportChain(ctx context.Context, file *adminpb.File) (*commonpb.IsOk, error) {
...
func (api *ChainManager) ImportChain(ctx context.Context, file *adminpb.File) (*commonpb.IsOk, error) {
    ...
    for batch := 0; ; batch++ {
        ...
        for len(blocks) < cap(blocks) {
            ...
            if err := stream.Decode(block); err == io.EOF {
                ...
                return &commonpb.IsOk{IsOk:false}, fmt.Errorf("block %d: failed to parse: %v", index, err)
            }
        ...
        if _, err := api.c.BlockChain().InsertChain(blocks); err != nil {
            return &commonpb.IsOk{IsOk:false}, fmt.Errorf("batch %d: failed to insert: %v", batch, err)
        }
        blocks = blocks[:0]
    }
    return &commonpb.IsOk{IsOk:true}, nil
