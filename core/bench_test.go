...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, dpor.NewFaker(configs.ChainConfigInfo().Dpor, db), db, remoteDB, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, dpor.NewFaker(configs.ChainConfigInfo().Dpor, db), vm.Config{}, remoteDB, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, configs.TestChainConfig, dpor.NewFaker(configs.ChainConfigInfo().Dpor, testdb), vm.Config{}, remoteDB, nil)
		if err != nil {
