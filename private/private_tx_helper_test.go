...
func (adapter *FaultIpfsAdapter) Cat(path string) (io.ReadCloser, error) {
	return nil, errors.New("Fault IPFS")
}

func (adapter *FaultIpfsAdapter) Add(r io.Reader) (string, error) {
	return "", errors.New("Fault IPFS")
}
...
func callSealPrivatePayload(t *testing.T, remoteDB ethdb.RemoteDatabase) {
	...
	replacement, err := SealPrivatePayload(payload, txNonce, parties, remoteDB)
	if err != nil {
