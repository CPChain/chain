...
func TestRetrieveAndDecryptPayload(t *testing.T) {
	...
	type args struct {
		...
		accountBasedDecryptor accounts.AccountBasedDecryptor
	}
	tests := []struct {
		name              string
		args              args
...
func (self *fakeAccountBasedDecryptor) CanDecrypt(pubkey string) (canDecrypt bool, wallet accounts.Wallet, account *accounts.Account) {
	return self.publicKey == pubkey, fakeWallet{}, &accounts.Account{}
}
...
func getDecryptor() accounts.AccountBasedDecryptor {
